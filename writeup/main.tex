...
The analysis presents an SCM model, an exploration of ASCM for bias correction, and a DiD setup to challenge the choiche of the SCM model. 
...
The next plot shows the the donor weights used to construct the synthetic McDonald’s. It shows which firms contribute to the synthetic control and by how much.
...
ASCM (augmented SCM) is designed for cases where SCM cannot achieve good pre-treatment fit. It “bias-corrects” imperfect matching by adding an outcome model, often ridge regression, that adjusts for remaining pre-treatment imbalance. Hence, this section explored wether ASCM could improve the pre-treatment fit, by reducing residual mismatch bias.
ASCM does not solve a structural break problem. COVID likely changed the underlying mapping between McDonald’s and the donor pool. ASCM also cannot fix a bad donor pool problem, because if the donors are structurally different (some are from retail, subscription services, or hospitality), the model may end up fitting pre-periods but still extrapolate in a way that has no causal meaning. However, 
