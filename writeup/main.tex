...
This analysis' scraping file extracts the raw company concepts with all the tags available for the firms of interest. The preprocessing part loads the raw financial observations then resolves the multiple reporting sources by selecting a dominant source tag. The data are then restricted to valid quarterly observations, 

after which conflicts across reporting intervals are resolved by retaining the observation with the longest reporting window. THIS IS WEIRD ACTUALLY  
