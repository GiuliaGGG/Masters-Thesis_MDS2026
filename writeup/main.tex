...
\begin{figure}[htbp]
    ...
    \caption{Treatment Status PRE IMPUTATION}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{images/c83be82a-d34f-48dd-b5b6-0169ea44a1d4.png}
    \caption{Treatment Status POST IMPUTATION}
    \label{fig:mcd_revenue}
\end{figure}
...
Let us pan out what are the characteristics of the case at hand. The addition of mcDonalds on the boycott list provides us with a pre and port treatment period. The setup presents a small number of treated units (2 years) and long pre-treatment histories (from 2008 to 2203, so 17 years). There is a treated unit, McDonald, that receives a treatment that not every company receives. Mcdonalds has several characteristics in common with other firms in the restauration and fast food business, they are subjected to similar demands from consumers as they fill a similar market and can be assumed to have a similar seasonality, they are impacted from similar economic shocks and inflation levels. Hence, they are somehow comparable. However, a comparison between McDonalds revenue and all the other firms' revenue would not be informative, because there are several other aspects that differentiate a a fast food from another, like different success among costumers, that make the revenues between different firms not comparable, or more specifically, not parallel before treatment.

The treatment, boycott call, is complex, endogenous, and non-linear

This setup may allow a typical Difference in Difference (DiD) analysis, which compares the change in outcomes over time in a treated group to the change in outcomes over time in a control group,
...
From the BDS page, the mcdonalds company is listed under grassroots organic boycott targets. The organic boycott targets are campaigns that the BDS movement did not initiat is in support of them due to these brands openly supporting Israel’s genocide against Palestinians (\cite{DS2025CompaniesGenocide}). The companies McDonald’s (US), Burger King (US), Papa John’s (US), Pizza Hut (US), WIX (Israel) have been targeted as of November the 5th by the bds, after being targeted in some countries by grassroots organic boycott campaigns, not initiated by the BDS movement, because these companies, or their branches or franchises in Israel, have openly supported Israel and/or provided generous in-kind donations to the Israeli military. Quoting : " If these grassroots campaigns are not already organically active in your area, we suggest focusing your energies on our strategic campaigns above." 

This research acklowledges that BDS did not start the boycott, and that since the adding the boycott on the website came very close to the start of the boycott, it might well be that the fact that BDS added the target on their page had no impact on the boycott themselves. However, taking the BDS call as a treatment is a way of centralizing this otherwise scattered treatment of a boycott call, and based on the available literature it is fair to assume that boycott movements need some sort of centralization to be effective. The twitter post is from the 19th of october 2023, and the BDS adding the result is on the 5th of November. This time difference can't be captured by the quarterly data. 
...
\begin{minipage}{\textwidth}
    ...
    \\{\footnotesize (a) Guide to BDS Boycott - November 2023}
\end{minipage}
...
The criteria to select treatment are based on the BDS page rationale for which companies become a boycott target, and are level of complicity, potential for cross-movement coalition-building, media appeal, and potential for success (\textcite{bdsStrategic2024}). The control units are all companies that are big enough to be on SEC, and that are US based. This is a first way to ensure that they are all comparable.
...
The pre-treatment period is defined as the time before the BDS movement called for a boycott against McDonald's, which occurred in November 2023. The wayback machine was manually checked to ensure that the boycott call was present since then, and not before, and also that the control group companies were not called for boycott at any point in time. The post-treatment period starts from the last quarter of 2023 and ends when the latest available data point, which is in Q3 2025.

\section{Analysis}

The analysis is conducted using the gsynth package, which implements the Generalized Synthetic Control Method (GSCM) developed by Xu (\citeyear{xu2017generalized}). The package imputes counterfactuals for each treated unit using control group information based on a linear interactive fixed effects model that
incorporates unit-specific intercepts interacted with time-varying coefficient (\textcite{Xu2025gsynthManual}). The package generalizes the synthetic control method to the case of multiple treated units and variable treatment periods, which is not relevant for this analysys, as there is just one treatment period and one treated unit. The reason why this package was used over the classic one used in Abadie because it is more effcicient and interpretable. It allows the treatment to be correlated with unobserved unit and time heterogeneities (under reasonable modeling assumptions), while synth relies on matching pre-treatment outcomes and covariate. With a built-in cross-validation procedure, it avoids specification searches that would be required in synth where researchers choose predictors and weights manually, and thus is easier to implement. 

The analysis includes both unit and time fixed effects, which control for time-invariant firm characteristics and common time shocks, while allowing gsynth to capture additional latent factors. The fact that different companies have different scales is assumed to be invarint over time, and partly controlled for prior to this step by taking the ratios to a baseline value in the preprocessing, but in this part of the analysys a further control is imposed as the model first removes all differences that are constant for each unit, and all shocks that are common to all units at a given time, like inflation or recessions. A cross validation procedure selects the optimal number of unobserved confounders, instead of fixing them arbitrarily. 
